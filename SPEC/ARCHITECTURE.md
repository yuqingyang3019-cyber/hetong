...
- 前端负责钉钉客户端 SDK 能力，包括免登授权码获取、合同结果预览和用户可见交互。
- BFF 只负责前端配套能力，包括静态资源、公开配置、钉钉免登服务端换取、会话维护和 AgentRun 访问凭证签发。
- AgentRun 只处理业务数据，包括报价单上传、解析、字段识别、合同生成、服务端 SDK 上传钉盘和返回预览链接。
...
| 逻辑层 | 物理实现 | 主要职责 |
| --- | --- | --- |
| 前端 H5 | `frontend/src/app.js`、`frontend/src/index.html`、`frontend/src/app.css` | 页面交互、任务列表、用户确认、钉钉 JSAPI 免登和钉盘预览 |
| BFF 鉴权层 | `frontend/src/server.py` | 静态资源、`/config.js`、钉钉免登服务端换取、H5 会话、AgentRun 短期凭证签发 |
...
- 使用钉钉客户端 SDK 打开钉盘合同预览，预览页自带下载能力。
...
```mermaid
flowchart TD
  openApp["打开钉钉 H5"] --> auth["JSAPI获取免登码"]
...
| 鉴权边界 | BFF 负责免登和 AgentRun 短期凭证，AgentRun 只处理业务请求 | 当前实现仍由 AgentRun 处理免登并依赖 Cookie，H5 通过 BFF 代理业务接口 | 将免登迁移到 BFF，新增 AgentRun Bearer 鉴权 |
| 业务请求路径 | 前端直连 AgentRun 业务接口 | 当前实现仍经 H5 BFF 同源代理 `/api` 和 `/ag-ui` | 前端改为使用 AgentRun baseUrl + Bearer Token |
| 合同交付 | 前端使用钉钉 JSAPI 预览钉盘文件 | 当前实现支持代理下载或打开 `openUrl` | 返回稳定预览链接，前端接入钉钉文件预览能力 |
