...
| 适用范围 | 钉钉 H5、ECS 鉴权与业务接口、报价单解析、合同生成、钉盘下载交付 |
...
鉴权与静态资源层由同一个 ECS FastAPI 服务提供，用于托管 H5 页面、完成钉钉免登、维护 H5 会话和签发业务访问凭证。它与业务 API 同域部署，不再需要独立 H5 BFF 函数。
...
主要配置由 ECS 的 `/opt/hetong-agui/.env` 注入；变量模板见 [`.env.ecs.example`](../.env.ecs.example)。
...
| 鉴权边界 | ECS 后端负责免登和短期业务凭证，业务 API 校验 Bearer Token | 同一 FastAPI 服务内 `/bff/auth/*` + Bearer 鉴权，钉钉调用使用官方新版 SDK | 在真实钉钉环境验证新版 SDK 免登字段稳定性 |
| 业务请求路径 | 前端同域调用 ECS 业务接口 | 相对路径 + Bearer Token | 部署时确保自定义域名指向单个 ECS 服务 |
| 合同交付 | 前端通过 ECS 下载钉盘文件 | 返回 `dingDrive` 和 `download` 结构，并由 ECS 代理下载合同文件 | 确认钉盘下载信息接口在真实环境的权限配置 |
