...
- AgentRun 只处理业务数据，包括报价单上传、解析、字段识别、合同生成、使用钉盘官方新版 SDK 上传合同和代理下载合同文件。
...
- 通过 AgentRun 下载接口下载钉盘合同文件，并提示用户保存位置。
...
flowchart TD
  ...
  cleanup --> download["AgentRun代理下载合同"]
```
...
7. AgentRun 上传合同到钉盘。
8. AgentRun 通过 AG-UI 事件返回进度、日志和钉盘文件信息。

### 6.4 钉盘下载流程

1. 前端从 `contract_generated` 事件获得钉盘 `spaceId`、`fileId` 和 `fileName`。
2. 用户点击下载后，前端携带 BFF 签发的 AgentRun Bearer Token 调用 `POST /api/dingdrive/download`。
3. AgentRun 调用钉盘 `GetFileDownloadInfo` 获取下载 URL 和 headers。
...
| `CUSTOM contract_generated` | 返回合同 ID、文件名、钉盘文件信息和下载提示 |
